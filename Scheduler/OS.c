...
static u8      task_number = 0u;               /**< Number of the registered tasks. */
 ...
 * @param   default_t_periodicity: The time it gets called periodically.
 ...
 */
OS_feedback OS_TaskCreate(fncPtr function, u8 default_t_periodicity, OS_state default_state)
{
    ...
    /* Time limit. */
    else if ((OS_MIN_TIME > default_t_periodicity) || (OS_MAX_TIME < default_t_periodicity))
    {
        ret = NOK_TIME_LIMIT;
    }
    /* Task number limit. */
    ...
    else
    {
        ...
        task_array[task_number].t_periodicity = default_t_periodicity;
...
{
    for (u8 i = 0u; i < task_number; i++)
    {
        /* Ignore SUSPENDED tasks. */
        if (SUSPENDED != task_array[i].state)
        {
            /* Put it into READY state. */
            if (task_array[i].t_periodicity <= task_array[i].time_cnt)
            {
...
{
    return task_array[task_number].t_periodicity;
}
 ...
 * @param   new_t_periodicity: The new burst time of the task.
 * @return  void
 */
void OS_SetTaskBurstTime(u8 task_number, u8 new_t_periodicity)
{
    task_array[task_number].t_periodicity = new_t_periodicity;
}
